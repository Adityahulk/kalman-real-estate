...
class ExportService {
  ...
  Future<File> writeCsvBundle(int projectId) async {
    ...
    final rows = <List<Object?>>[
      ...
      for (final buyer in data['buyers'] as List)
        ['BUYER', buyer['id'], buyer['name'], '', '', buyer['channelPartner']],
      for (final token in data['tokens'] as List)
        [
          'TOKEN',
          token['id'],
          token['tokenCode'],
      ...
      for (final plot in data['plots'] as List)
        [
          'PLOT',
          plot['id'],
          plot['plotNumber'],
      ...
      for (final history in data['history'] as List)
        [
          'HISTORY',
          history['id'],
          history['actionType'],
  ...
  Future<BackupValidationResult> validateBackupFile(File file) async {
    ...
    if (decoded is! Map<String, Object?>) {
      return const BackupValidationResult(
          isValid: false,
          message: 'Backup file is not a valid project backup.');
    }
    if (decoded['schemaVersion'] != db.schemaVersion) {
      return BackupValidationResult(
        isValid: false,
        message:
            'Backup schema ${decoded['schemaVersion']} does not match app schema ${db.schemaVersion}.',
      );
    }
    ...
    if (projectName is! String || projectName.trim().isEmpty) {
      return const BackupValidationResult(
          isValid: false, message: 'Backup is missing project name.');
    }
    ...
    if (payments is List) {
      for (final payment in payments.whereType<Map>()) {
        for (final key in ['holderSignaturePath', 'authorizedSignaturePath']) {
          ...
          if (signaturePath is String &&
              signaturePath.isNotEmpty &&
              !await File(signaturePath).exists()) {
    ...
    if (missingSignatures.isNotEmpty) {
      return BackupValidationResult(
        isValid: false,
        message:
            'Backup is readable, but ${missingSignatures.length} signature file(s) are missing on this device.',
      );
    }
    return BackupValidationResult(
        isValid: true,
        message: 'Backup for $projectName is valid.',
        file: file);
  }
  ...
  Future<Map<String, Object?>> _projectSnapshot(int projectId) async {
    final project = await (db.select(db.projects)
          ..where((tbl) => tbl.id.equals(projectId)))
        .getSingle();
    final buyers = await (db.select(db.buyers)
          ..where((tbl) => tbl.projectId.equals(projectId)))
        .get();
    final tokens = await (db.select(db.tokens)
          ..where((tbl) => tbl.projectId.equals(projectId)))
        .get();
    final plots = await (db.select(db.plots)
          ..where((tbl) => tbl.projectId.equals(projectId)))
        .get();
    final eoi = await (db.select(db.eoiForms)
          ..where((tbl) => tbl.projectId.equals(projectId)))
        .get();
    final payments = await (db.select(db.paymentEntries)
          ..where((tbl) => tbl.projectId.equals(projectId)))
        .get();
    final history = await (db.select(db.plotHistory)
          ..where((tbl) => tbl.projectId.equals(projectId)))
        .get();
    return {
...
class BackupValidationResult {
  const BackupValidationResult(
      {required this.isValid, required this.message, this.file});
