...
export default async function ProjectWorkspacePage({ params, searchParams }: { params: { projectId: string }; searchParams: { view?: string } }) {
  ...
  return <main className="flex min-h-[calc(100vh-4rem)] flex-col px-4 py-6 lg:px-8">
    <header className="shrink-0 border-b border-slate-200 pb-5"><BackButton fallbackHref="/app" /><div className="mt-2 text-sm text-slate-500">{project.state ?? "State not added"}</div><h1 className="mt-1 text-2xl font-semibold">{project.name}</h1><div className="mt-4 flex flex-wrap gap-2"><Link className={view === "details" ? "btn-primary" : "btn-outline"} href={`/app/projects/${project.id}?view=details`}><Building2 size={16} /> Project detail list</Link><Link className="btn-outline" href={`/app/projects/${project.id}/cad`}><Map size={16} /> Maps</Link><Link className="btn-outline" href={`/app/projects/${project.id}/files`}><FileStack size={16} /> Upload files</Link></div></header>
