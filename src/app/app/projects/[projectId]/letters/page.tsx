...
export default async function ProjectLettersPage({ params }: { params: { projectId: string } }) {
  ...
  const pdfTemplates = templates.filter((template) => !template.sourceFileId || sourceNames.get(template.sourceFileId)?.toLowerCase().endsWith(".pdf"));
  return <main className="min-h-[calc(100vh-4rem)] px-4 py-6 lg:px-8"><BackButton fallbackHref={`/app/projects/${project.id}`} /><div className="mb-5"><div className="text-sm text-slate-500">{project.name}</div><h1 className="mt-1 text-2xl font-semibold">Set your letters</h1><p className="mt-1 text-sm text-slate-500">Upload a PDF, select text to create fields, map them to system data, then save. Generated letters use your PDF wording with mapped values filled in.</p></div><LetterTemplateBuilder projectId={project.id} categories={categories.map((category) => ({ id: category.id, name: category.name, fields: category.fields.map((field) => ({ id: field.id, label: field.label, mapping: field.mapping })) }))} templates={pdfTemplates.map((template) => ({ id: template.id, name: template.name, body: template.body, sourceFileId: template.sourceFileId, sourceFileName: template.sourceFileId ? sourceNames.get(template.sourceFileId) : undefined, fields: templateFields(template.variables), createdAt: template.createdAt.toISOString() }))} /></main>;
}
