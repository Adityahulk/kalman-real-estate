...
export function NewLeadForm({ campaigns, projects, referrals, sources, users }: { campaigns: Option[]; projects: Option[]; referrals: Option[]; sources: Option[]; users: UserOption[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const lead = await crmRequest<{ id: string }>("/api/v1/crm/leads", "POST", {
        ...
        clientType: text("clientType") || null,
        ...
        priceDiscussedInr: number("priceDiscussedInr"), offersDiscounts: text("offersDiscounts") || null,
        paymentPreference: text("paymentPreference") || null, negotiationStatus: text("negotiationStatus") || null,
        mainObjections: text("mainObjections") || null, competitorComparison: text("competitorComparison") || null,
        requestedInformation: text("requestedInformation") || null,
  ...
  return <form className="mx-auto max-w-6xl" onSubmit={submit}>
    ...
    <div className="space-y-6">
      <section className="rounded-lg border border-slate-200 bg-white p-5"><h2 className="font-semibold">Customer</h2><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3"><Field label="Name" name="name" required/><Field label="Primary phone" name="primaryPhone" required/><Field label="WhatsApp number" name="whatsappPhone"/><Field label="Alternate phone" name="alternatePhone"/><Field label="Email" name="email" type="email"/><Field label="City" name="city"/><Field label="Area / locality" name="area"/><Select label="Client type" name="clientType" options={["Investor","End User","Dealer","Other"]}/><Select label="Preferred language" name="preferredLanguage" options={["Punjabi","Hindi","English"]}/><Select label="Preferred contact" name="preferredContactMethod" options={["Phone","WhatsApp","Email"]}/></div></section>
      <section className="rounded-lg border border-slate-200 bg-white p-5"><h2 className="font-semibold">Project opportunity and sales brief</h2><p className="mt-1 text-sm text-slate-500">If this person already exists, choosing a different project creates a linked opportunity without duplicating the client.</p><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3"><SelectOptions label="Lead source" name="sourceId" options={sources} required/><SelectOptions label="Campaign" name="campaignId" options={campaigns}/><SelectOptions label="Referred by" name="referredByLeadId" options={referrals}/><SelectOptions label="Interested project" name="interestedProjectId" options={projects} required/><Select label="Property type" name="propertyType" options={["Plot","Shop","Flat","Commercial","Other"]}/><Field label="Specific property / plot" name="interestedProperty"/><Field label="Budget from (₹)" name="budgetMinInr" type="number"/><Field label="Budget to (₹)" name="budgetMaxInr" type="number"/><Field label="Purchase timeline" name="purchaseTimeline" placeholder="e.g. Within 3 months"/><Field label="Purpose" name="purpose" placeholder="Investment, residence, business..."/><Field label="Price discussed (₹)" name="priceDiscussedInr" type="number"/><Field label="Offers / discounts" name="offersDiscounts"/><Field label="Payment preference" name="paymentPreference"/><Field label="Negotiation status" name="negotiationStatus"/><Field label="Main objections" name="mainObjections"/><Field label="Competitor / project comparison" name="competitorComparison"/><Field label="What the client asked for" name="requestedInformation"/><Field label="Previous work / reference" name="previousWork"/></div></section>
