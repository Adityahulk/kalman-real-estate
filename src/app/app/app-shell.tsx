      ...
      >
        ...
        <nav className="flex-1 space-y-1 p-3">
          {moduleNav.filter((item) => (
            item.key === "workspace" ? access.projects : item.key === "ownership" ? access.ownership : access.development
          )).map((item) => (
            <ShellLink
              key={item.key}
          ...
          {access.marketing ? globalNav.map((item) => (
            <ShellLink
              key={item.href}
              href={item.href}
              label={item.label}
          ...
          {access.restore ? (
            <ShellLink
              ...
              icon={ArchiveRestore}
              collapsed={collapsed}
              active={pathname.startsWith("/app/archive")}
            />
          ) : null}
        </nav>
