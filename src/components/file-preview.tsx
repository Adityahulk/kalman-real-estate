...
export function FilePreview({ id, fileName, mimeType }: { id: string; fileName: string; mimeType: string }) {
  ...
  return <div className="flex h-[calc(100vh-16rem)] min-h-[420px] flex-col bg-slate-50">
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-3 py-2">
      ...
      <div className="flex gap-1">
        <a className="btn-outline h-8 px-2 text-xs" href={url} target="_blank" rel="noreferrer"><ExternalLink size={13} /> Open</a>
    ...
    <div className="pdf-scroll-viewer min-h-0 flex-1 overflow-auto">
      ...
      {isPdf ? <iframe className="h-full min-h-[640px] border-0 bg-white" src={`${url}#zoom=${zoom}`} title={fileName} scrolling="yes" style={{ width: `${Math.max(100, zoom)}%` }} /> : null}
